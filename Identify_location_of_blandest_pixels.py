...
def location_of_bland(image_id, masked_image, samples, BD530_2, BD860_2, BD1400, OLINDEX3, LCPINDEX2, HCPINDEX2,
    ...
    def OtherBlandParams(column_number):
        indices = (np.array(np.where(samples_list == column_number)))[0]
        BD530_2_in_column = []
        BD860_2_in_column = []
        BD1400_in_column = []
        ...
        return new_index_of_blandest
    
    new_index_of_blandest = []
    for number in range(len(cols)):
        try:
            new_index_of_blandest.append(OtherBlandParams(number))
        except:
            try:
                exception1(number)
            except:
    ...
    all_indices_of_bland = list(itertools.chain.from_iterable(new_index_of_blandest))
    location_of_bland = []
    for i in tqdm.tqdm(range(len(BD2290_list))):
        if i in all_indices_of_bland:
            location_of_bland.append(1)
        else:
            location_of_bland.append(0)
            
    location_of_bland_reshaped = np.reshape(location_of_bland, ((whole_image.shape[0],whole_image.shape[1])))
    path = #path to save data
    cv2.imwrite(path, location_of_bland_reshaped)
    path = #path to save data
    im = Image.fromarray((location_of_bland_reshaped*255).astype(np.uint8))
    im.save(path)
     

    return location_of_bland_reshaped
